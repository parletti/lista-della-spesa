...
export function setSessionStartedCookie() {
  ...
  document.cookie = `${SESSION_STARTED_AT_COOKIE}=${now}; Max-Age=${maxAge}; Path=/; SameSite=Lax${cookieFlags()}`;
}

export function clearSessionStartedCookie() {
  if (typeof document === "undefined") return;
  document.cookie = `${SESSION_STARTED_AT_COOKIE}=; Max-Age=0; Path=/; SameSite=Lax${cookieFlags()}`;
}
